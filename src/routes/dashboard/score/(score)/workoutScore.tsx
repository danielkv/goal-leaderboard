...
const WorkoutScorePage: React.FC = () => {
          ...
          <SelectableFilter<Score, 'status'>
            ...
            filters={[
              { label: 'Todos', name: 'all', count: data.length },
              {
                ...
                count: data.filter(
                  (item) => item.status === ScoreStatus.PUBLISHED,
                ).length,
              ...
              },
              {
                ...
                count: data.filter(
                  (item) => item.status === ScoreStatus.NON_PUBLISHED,
                ).length,
                icon: <WatchLater fontSize="small" color="warning" />,
              },
              {
                label: 'Pendentes',
                name: ScoreStatus.PENDING,
                count: data.filter(
                  (item) => item.status === ScoreStatus.PENDING,
                ).length,
                icon: <RemoveCircleOutline fontSize="small" color="disabled" />,
              },
            ]}
          />
        </Box>
...
export const Route = createFileRoute('/dashboard/score/(score)/workoutScore')({
